<script lang="ts" module>
  ...
  export const buttonVariants = tv({
    ...
    variants: {
      variant: {
        default:
          "bg-radial-[at_52%_-52%] **:[text-shadow:0_1px_0_var(--color-primary)] border-primary from-primary/70 to-primary/95 text-primary-foreground inset-shadow-2xs inset-shadow-fg/10  dark:hover:to-primary border text-sm shadow-md shadow-zinc-950/30 ring-0 transition-[filter] duration-200 active:brightness-95 dark:border-0",
        destructive:
          "bg-destructive text-destructive-foreground hover:bg-destructive/90 shadow-xs",
        outline:
          "shadow-xs hover:bg-muted  bg-background dark:bg-background dark:hover:bg-muted/50 dark:border-border border border-zinc-300 shadow-zinc-950/10 duration-200",
        secondary:
