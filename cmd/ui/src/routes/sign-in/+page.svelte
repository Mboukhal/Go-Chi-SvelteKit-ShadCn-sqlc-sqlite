...
//   let messages = $state()
	...
	let errorField: string | undefined = $state();
	let formData = $state({	
		email: "",
		pwd: "",
	});
	...
	onMount(() => {
		...
		if (error) {
			...
			setTimeout(() => {
				toast.error(decodedError || "An error occurred during sign-in", { duration: 10000 });
			}, 100);
		}
	});
	...
	// 		// TODO: Redirect to login or verify email page
	// 	} catch (err) {
...
<section class="flex min-h-screen px-4 py-16 md:py-32 z-50">
	...
	<div class="m-auto h-fit w-full max-w-sm overflow-hidden rounded-xl border shadow z-50">
		...
		<!-- <div class="p-8 pb-6"> -->
		<div class="bg-card/80 -m-px rounded-xl border p-6 space-y-8">
			...
			<form class="flex gap-4 items-center" method="POST" action="/api/v1/auth/magic-link/request">
